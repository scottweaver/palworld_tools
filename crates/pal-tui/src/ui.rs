...
pub fn draw(frame: &mut Frame, app: &App) {
    ...
    frame.render_widget(
        Paragraph::new(
            "Tab panes · type filter · Enter/click select · F4/⇧click progenitor · ^D clear/delete · ←/→ depth · h/a/d IV min (⇧ lowers) · F2 wild · F5 search · F6 reload pool · ^S save plan · ^L library · : commands (:help) · Esc quit",
        )
