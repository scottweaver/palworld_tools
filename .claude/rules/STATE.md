...
|---|---|---|
| `main` | trunk | at `8f46a70`, CI green, 31 tests |
...
2. Solver refinements toward palcalc parity (wild pals, IVs,
   ...
   precompute `SpeciesAdjacency` once per session, search off the UI
   thread — matters more now that depth is user-adjustable).
